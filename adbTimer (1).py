...
while iterations<=maxIterations:
 ...
 #               psu.write('OUTP ON')
        ...
        if (curr_val >= currThreshold) and (not spikeDetected):
            spikeDetected = True
            spikeTime = timeElapsed
            print("\n Current spike detected, press any key to stop logging")
        ...
        if spikeDetected and msvcrt.kbhit():
            msvcrt.getch() #get the key
            print("Logging stopped")
            testing = False

        if spikeDetected and (timeElapsed - spikeTime >= secondsAfterSpike):
            print("Logging stopped")
            testing = False

    print(f"Test #{iterations} complete.\n")
